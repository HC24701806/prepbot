...
module.exports = {
	...
	async execute(interaction) {
        ...
        if(!(await getDoc(docRef)).exists()) {
            ...
            let index = 0
            if(test == '') {
                test = allowed[Math.floor(Math.random() * (allowed.length - 1))]
            }
            
            if(test == 'amc') {
                index = Math.floor(Math.random() * 2300) + 1
            } else if(test == 'aime') {
                index = Math.floor(Math.random() * 945) + 10001
            }
                ...
                if(Object.keys(reviewProbs).includes(indexStr)) {
                    if(code && code == 'correct') {
                        if(reviewProbs[indexStr] == 2) {
                            reviewProbs[indexStr] = 1
                        } else {
                            delete reviewProbs[indexStr]
                        }
                    } else {
                        reviewProbs[indexStr] = 2
                    }
                } else {
                    reviewProbs[indexStr] = 2
                }
